...
/**
 * 071-now playground admin E2E specs (Issue #141; supersedes the
...
test.describe( 'Playground admin', () => {
	test( 'admin opens already logged in (auto-login)', async ( { page } ) => {
		...
		// Open the post editor. Auto-login means it opens straight
	...
	test( 'creates and edits a post through the admin forms', async ( {
		...
		// Create a post through the admin's own form. The form
		...
		// The created post shows on the front page.
		...
		// Edit the post. The admin lists an "Edit" link per post --
		...
		// The edited title shows on the front page.
		await gotoBlog( page, '/index.php' );
		expectTrue(
	...
	test( 'adds a category through the category admin', async ( { page } ) => {
		...
		// Add a category through the category admin and confirm it
