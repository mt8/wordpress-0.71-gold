/*
 * Unit tests for tools/env/src/mappings.mjs -- rendering the runtime Compose
...
test( 'renderMappingsOverride: a mapping is written host:container (not reversed)', () => {
	// The config maps container path -> host path; the volume line is
...
test( 'writeMappingsOverride: with mappings writes the file and returns its path', () => {
	// Clean up first in case a previous run left the file.
...
test( 'writeMappingsOverride: with no mappings removes a stale file and returns null', () => {
	try {
		// Seed a stale file, then assert an empty-mappings call clears it.
