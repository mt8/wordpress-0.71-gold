...
switch ( $action ) {
	...
	case 'post':
		...
		// CSRF check -- reject a forged request to create a post.
		b2_csrf_check( 'post' );
	...
	case 'edit':
		...
		// Cast the post id to int -- it is used unquoted in SQL (WHERE ID=$post).
		$post = (int) $_GET['post'];
		if ( $user_level > 0 ) {
			$postdata   = get_postdata( $post ) or die( 'Oops, no post with this ID. <a href="b2edit.php">Go back</a> !' );
			$authordata = get_userdata( $postdata['Author_ID'] );
			// Ownership check -- a user may edit a post only if they are
	...
	case 'editpost':
		...
		// CSRF check -- reject a forged request to edit a post.
		...
		// Cast the post id to int -- it is used unquoted in SQL (WHERE ID = $post_ID).
		$post_ID = (int) $_POST['post_ID'];

		// Ownership check -- load the real post row and verify the user
	...
	if ( ( $postdata['Author_ID'] != $user_ID ) && ( $user_level <= $authordata->user_level ) ) {
		...
		$post_category = intval( $_POST['post_category'] );
		// Issue #60 -- the edit form (b2edit.form.php) never renders a
	...
	case 'delete':
		...
		// CSRF check -- reject a forged GET request to delete a post.
		...
		// Ownership check -- a user may delete a post only if they are
		// its author, or their level is strictly higher than the
	...
	default:
		...
		if ( $user_level > 0 ) {
			if ( ( ! $withcomments ) && ( ! $c ) ) {
				...
				// Issue #96 -- entry point to create a new post in the
