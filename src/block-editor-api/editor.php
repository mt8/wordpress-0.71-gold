 ...
 *     bundled custom block editor (built from ../block-editor-prototype/ by
 *     `npm run build` into ../block-editor-assets/) and mounts it for a
 *     chosen post id. Usage: open editor.php?post=ID in a browser while
 *     logged in to WordPress 0.71's admin.
 * JA: Issue #65 の実験的試作。バンドル済みのカスタムブロックエディタ
 *     (../block-editor-prototype/ から `npm run build` で
 *     ../block-editor-assets/ へビルド)を読み込み、選んだ投稿 ID に対して
 *     マウントする HTML シェルを配信する。使い方: WordPress 0.71 の管理画面に
 *     ログインした状態で、ブラウザで editor.php?post=ID を開く。
 *
...
$assets_dir   = __DIR__ . '/../block-editor-assets';
$manifest_url = '../block-editor-assets/.vite/manifest.json';
...
$js_url = ( is_array( $entry ) && isset( $entry['file'] ) ) ? '../block-editor-assets/' . $entry['file'] : null;
...
<link rel="stylesheet" href="../block-editor-assets/<?php echo htmlspecialchars( (string) $href, ENT_QUOTES ); ?>" />
<?php endforeach; ?>
<style>
