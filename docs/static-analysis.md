...
Two static-analysis tools are wired up: phpcs (WordPress coding style) and
...
phpcs runs the official **WordPress-Core** coding standard
...
As of Issue #49, **phpcs reports 0 errors and 0 warnings**. The
...
A `PHPCompatibility` (target PHP 8.3) standard was also run during the
...
Deeper static analysis (undefined variables, type errors). Configured by
...
The `composer phpstan` script passes `--memory-limit=1G` -- PHPStan's
default memory limit can OOM-crash on some machines (Issue #71).

As of Issue #22, **PHPStan level 0 reports 0 errors with no baseline**. The
...
Raising the level above 0 is out of scope: level 1 alone adds roughly 1050
...
A git `pre-commit` hook keeps the two checks above from regressing
...
On every commit, `.husky/pre-commit` runs `lint-staged`. When the commit
...
`.husky/pre-commit` is committed **executable** (mode 100755) with a
...
2 つの静的解析ツールを導入している: phpcs(WordPress コーディングスタイル)と
...
phpcs は公式の **WordPress-Core** コーディング標準
...
Issue #49 時点で **phpcs はエラー 0 件・警告 0 件**。`WordPress-Core` 標準は
...
移行期間中は `PHPCompatibility`(対象 PHP 8.3)標準も実行しており、0 件に
到達した(Issue #13・#22)。PHP 8.3 移行の完了後、phpcs 設定から除去した
(Issue #49)。現在 phpcs は WordPress コードスタイルのみを検査する。

## PHPStan

より深い静的解析(未定義変数・型エラー)。`phpstan.neon.dist` で設定: level 0、
...
`composer phpstan` スクリプトは `--memory-limit=1G` を渡す -- PHPStan の
既定のメモリ上限は環境によって OOM クラッシュしうる(Issue #71)。

Issue #22 時点で **PHPStan level 0 は baseline なしで検出 0 件**。レガシー
...
level を 0 より上げることはスコープ外: level 1 だけで WordPress 0.71 の
...
git の `pre-commit` フックにより、上記 2 つの検査が気づかれぬまま退行する
...
コミットのたびに `.husky/pre-commit` が `lint-staged` を実行する。
...
`.husky/pre-commit` は `#!/usr/bin/env sh` シバン付きで**実行権限付き**
