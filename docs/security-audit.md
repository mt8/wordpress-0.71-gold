...
EN: A whole-codebase security review of WordPress 0.71-gold. Every item below
...
JA: WordPress 0.71-gold の全体セキュリティレビュー。以下の項目はいずれも
...
| # | Category / 分類 | Severity / 深刻度 | Issue |
|---|---|---|---|
| 1 | SQL injection / SQL インジェクション | High | #31 |
| 2 | Cross-site scripting (XSS) / クロスサイトスクリプティング | High | #32 |
| 3 | Cross-site request forgery (CSRF) / CSRF | High | #33 |
| 4 | Authentication & session / 認証・セッション管理 | Critical | #34 |
| 5 | Access control / アクセス制御・認可 | Medium | #35 |
| 6 | File upload / ファイルアップロード | High (※) | #36 |
| 7 | Information disclosure & misc / 情報漏洩・その他 | Medium | #37 |

EN: (※) The file-upload code is vulnerable but `$use_fileupload` is `0` by
...
### 1. SQL injection (#31)
EN: All queries are built by string interpolation; no prepared statements.
...
JA: 認可の適用が不統一。`b2team.php` のユーザー管理ハンドラは相対レベル
比較のみで最低レベルゲートが無く、UI はリンクを別判定で隠すだけ。
オブジェクト ID は GET 由来で所有者チェックも部分的(IDOR)。

### 6. File upload (#36)
EN: `b2upload.php` builds the save path from the user-supplied filename
...
JA: `b2upload.php` は保存パスをユーザー指定ファイル名から組み立て(パス
...
JA: メールヘッダインジェクション(コメント投稿者制御の `From:` ヘッダ、
...
EN: The seven categories share many source files (`b2edit.php` alone appears
...
JA: 7 カテゴリは多くのソースファイルを共有する(`b2edit.php` だけで #31・
