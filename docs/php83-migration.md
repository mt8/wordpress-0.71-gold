# PHP 8.3 Migration Log

This document records the changes made to run WordPress 0.71-gold on PHP 8.3.
It is updated per Issue as the migration progresses.

---

## Issue #1: Fix PHP 8.3 syntax (parse) errors

First step of the strategy in CLAUDE.md — fix every location that produces a
...
Curly-brace string offset access syntax was removed in PHP 8.0.
```diff
...
Call-time pass-by-reference (`&` on a call argument) was removed in PHP 5.4.
...
`php -l` reports `No syntax errors detected` for all 52 PHP files.
```sh
...
PHP 8 behavioral changes that are not parse errors (e.g. old-style
constructors in the `wpdb` / `POP3` classes, removed functions) are handled in
later compatibility-phase Issues.

---

## Issue #5: Migrate ext/mysql to mysqli via a compatibility shim

WordPress 0.71 uses the ext/mysql API (`mysql_*`), removed in PHP 7.0 —
...
#### New file: `src/b2-include/mysql-shim.php`

Defines the 16 `mysql_*` functions in use as thin `mysqli` wrappers. It:
...
Requires the shim at the top of the file. The old-style constructor
...
A test script run inside the Docker `web` container against the `db`
...
`define('OBJECT', ..., true)` in `wp-db.php` emits an `E_WARNING` under
...
## Issue #7: Get the WordPress installer running on PHP 8.3 + MySQL 8

First concrete, verifiable milestone — running `wp-admin/wp-install.php`
...
WordPress 0.71's installer SQL relies on the permissive 2003-era MySQL
...
Line 5 read the installer step from `$HTTP_GET_VARS['step']`. That
...
In the Docker environment, starting from an empty database,
...
The `$HTTP_*_VARS` superglobals in other files, `get_magic_quotes_gpc()`,
`ereg`/`each` and other removed functions, and rendering the blog front end /
admin pages are later Issues.

---

## Issue #9: Replace functions and variables removed in PHP 7/8

Toward rendering the blog front end and admin pages, four classes of removed
...
New file `src/b2-include/php-compat.php` defines a `get_magic_quotes_gpc()`
...
The POSIX regex functions (removed in PHP 7.0) are reimplemented over PCRE
in `php-compat.php`. POSIX patterns are wrapped with a `~` delimiter; PCRE
errors degrade gracefully.

#### 3. `each()` shim

`each()` (removed in PHP 8.0) is reimplemented in `php-compat.php` using
`key()`/`current()`/`next()`.

#### 4. `$HTTP_*_VARS` -> superglobals

The `$HTTP_*_VARS` superglobals (removed in PHP 5.4) are mechanically
...
Verifying the blog front end revealed a 5th blocker: `apply_filters()`
...
In the Docker environment with WordPress installed, the blog front end
(`index.php`) and `b2login.php` both return HTTP 200 with **no fatal error**.

### Known remaining issues

The blog front end renders without a fatal error, but the seeded post is not
...
## Issue #11: Make the blog front end display posts

Follow-up to Issue #9. Two blockers stopped the blog from displaying the
seeded post.
...
PHP 8.0 changed string-to-number comparison: `0 == ''` is now false. The
...
With the SQL fixed, the post was queried but its content rendered empty.
...
The remaining `/e` `preg_replace()` modifiers — the `%u` decoder and the
...
## Issue #13: Replace the compatibility shims with native rewrites, add static analysis

Static analysis tooling was introduced, and its findings led to dropping the
...
PHPCompatibility (testVersion 8.3) violations dropped from **234 to 14**;
...
The runtime `E_WARNING` / `E_DEPRECATED` notices (undefined variables,
...
## Issue #15: Clean up the runtime warnings on the blog front end

After Issue #13 the blog front end still emitted ~46 non-fatal
...
In the Docker environment `http://localhost:8080/` returns HTTP 200,
displays the seeded post, and produces **zero** `Warning` / `Deprecated` /
`Notice` lines, with no fatal error and no SQL error.

### Out of scope

Warnings on admin pages beyond what the front end exercises, the remaining
`/e` modifiers, and the `POP3` old-style constructor are later Issues.

---

## Issue #19: Fix the b2register.php 404 and the login/feed warnings

A Playwright link check found the blog front-end links fully clean, but the
...
A Playwright pass over the 11 internal links from the front page reports,
...
## Issue #22: Bring phpcs and PHPStan (level 0) to zero

Finish the static-analysis cleanup: phpcs from 11 to 0, PHPStan level 0 from
215 to 0, and delete the PHPStan baseline so both tools are completely clean
without any suppression.

### Changes

phpcs (PHPCompatibility) findings, 11 → 0:
...
PHPStan level 0 findings, 215 → 0:
...
The XML-RPC endpoint (`xmlrpc.php`) has remaining PHP 8 *runtime*
...
## Issue #24: Fix runtime warnings on the admin screens

Logged in as admin and swept the WordPress 0.71 admin screens. No
...
Logged in as admin, every admin screen -- b2edit.php (new post / edit /
...
The blog front end's category / archive pages still emit two pre-existing
...
## Issue #26: Fix the two front-end runtime warnings

Follow-up to Issue #24. A sweep of the blog front end (home, category,
...
Every front-end page -- home, `?cat=`, `?m=` (year and year+month),
...
## Issue #28: Posting from the admin fails to redirect

Testing "post from the admin" found that posting does insert the row, but
...
From the admin, creating a post (publish and draft), editing a post
...
## Issue #31: Fix SQL injection in numeric id contexts

A security audit (Issue #31) found that WordPress 0.71 builds every SQL
...
A full migration to `mysqli` prepared statements / bound parameters is the
...
## Issue #32: Escape reflected user input in HTML output

A security audit (Issue #32) found several reflected XSS sites where
...
Post content rendered by `the_content()` and comment content rendered by
...
## Issue #33: Add CSRF protection to state-changing admin actions

A security audit (Issue #33) found that WordPress 0.71 has no CSRF
...
The **public comment form** (`b2comments.post.php`, `b2comments.php`,
...
## Issue #34: Authentication & session management

WordPress 0.71-gold stored and compared passwords in plaintext, emailed the
...
### A. Password hashing with transparent migration

New passwords are stored as bcrypt hashes via
...
### B. Auth-cookie consistency

`checklogin()` verifies the session by comparing the `wordpresspass` cookie
...
### C. Lost password becomes a reset

The `retrievepassword` case in `b2login.php` used to email
...
### D. Cookie security flags

All authentication `setcookie()` calls (`wordpressuser`, `wordpresspass`,
...
### E. Database column width

A bcrypt hash is 60 characters but `b2users.user_pass` was `varchar(20)`.
...
`php -l` passes on all four changed files; `composer phpcs` reports 0
...
Replacing the `md5(password-hash)` bearer cookie with a server-side random
...
## Issue #44: Remove the XML-RPC, comment, trackback and pingback features

### Summary

The XML-RPC server, the comment feature, trackback and pingback were
...
Thirteen files were removed with `git rm`:
...
Before removing each function the whole `src/` tree was grepped to confirm
...
### D. Functions intentionally kept

`make_url_footnote()` in `b2functions.php` was *kept* — despite living
...
`php -l` passes on every changed file and across all `src/**/*.php` (0
...
## Issue #35: Access control / authorization

A security audit (Issue #35) found that authorization was enforced
...
The new minimum-level gates run *after* the CSRF check added in Issue #33,
...
`php -l` passes on both changed files (`b2team.php`, `b2edit.php`) with 0
syntax errors. `composer phpcs` reports 0 violations (41 files).
`composer phpstan --memory-limit=1G` reports 0 errors (the default 128 MB
...
## Issue #36: File upload security

A security audit (Issue #36) found that `wp-admin/b2upload.php` built the
...
All changes are confined to `wp-admin/b2upload.php`.
...
`move_uploaded_file()` already only accepts a path that PHP itself
...
`php -l wp-admin/b2upload.php` passes with 0 syntax errors.
...
## Issue #37: Information disclosure & misc

A security audit (Issue #37) found three remaining lower-severity issues.
...
The audit asked to weigh hardening item C versus leaving it. The chosen
...
`php -l` passes with 0 syntax errors on all 17 changed files.
`composer phpcs` reports 0 violations (41 files). `composer phpstan
--memory-limit=1G` reports 0 errors (the default 128 MB OOMs on this
repository -- a pre-existing, unrelated constraint). Functional test against
...
## Issue #49: Raise phpcs to the WordPress Coding Standard

phpcs previously ran only `PHPCompatibility`. Issue #49 adds the official
...
After all three steps, `composer phpcs` reports **0 errors and 0 warnings**.

As a follow-up within this Issue, the `PHPCompatibility` standard (and the
...
| Sniff | Count | Fix |
|---|---|---|
| `WordPress.PHP.YodaConditions.NotYoda` | 232 | rewrote each `$var == literal` comparison to Yoda form `literal == $var` (behaviour-identical) |
| `PSR2.Classes.PropertyDeclaration` (`VarUsed` / `ScopeMissing`) | 24 | replaced the `var` keyword on `wpdb` properties with explicit `public` |
| `Squiz.Scope.MethodScope.Missing` | 13 | added explicit `public` visibility to every `wpdb` method |
| `PSR2.ControlStructures.SwitchDeclaration` | 14 | removed the `{ }` blocks wrapping `case`/`default` bodies |
| `Squiz.PHP.DisallowMultipleAssignments.FoundInControlStructure` | 9 | moved each assignment used inside an `if ( ... )` condition to its own statement |
| `Squiz.ControlStructures.ControlSignature.SpaceAfterCloseBrace` | 4 | joined `elseif`/`else` to the preceding closing brace |

### Excluded sniffs

These sniffs are excluded inside the `<rule ref="WordPress-Core">` block of
...
| Sniff | Why excluded |
|---|---|
| `WordPress.NamingConventions.ValidVariableName` / `ValidFunctionName` | snake_case renaming would rewrite the 2003-era b2/WordPress API (`the_ID()`, `balanceTags()`, …) |
| `PEAR.NamingConventions.ValidClassName` | wants `class wpdb` renamed to `Wpdb`; `wpdb` is a public API class |
| `Universal.NamingConventions.NoReservedKeywordParameterNames` | renaming a parameter changes the public function signature |
| `WordPress.Files.FileName` | renaming files ripples through every include/require and link |
| `WordPress.DB.PreparedSQL` | prepared statements; Issue #31 already hardened SQL with `(int)` casts |
| `WordPress.DB.RestrictedFunctions` | "use `$wpdb` not `mysqli_*`"; WordPress 0.71 IS the data layer and calls mysqli by design |
| `Universal.Operators.StrictComparisons` | `==`→`===` changes comparison strictness |
| `WordPress.PHP.StrictInArray` | adding `true` to `in_array()` changes comparison strictness |
| `WordPress.DateTime.RestrictedFunctions` | `date()`→`gmdate()` is a timezone behaviour change |
| `WordPress.PHP.NoSilencedErrors` | removing `@` changes runtime warning behaviour |
| `Generic.CodeAnalysis.AssignmentInCondition` | the `while ( $row = mysqli_fetch_*() )` fetch-loop idiom cannot be silenced with extra parentheses, and restructuring every loop risks infinite loops; the safe `if ( ... )` cases were fixed instead |

### Verification

`vendor/bin/phpcs` reports 0 errors and 0 warnings; `php -l` passes on all
...
## Issue #53: Add PHPUnit and a starter test suite

WordPress 0.71 shipped with no tests. Issue #53 adds PHPUnit (`^12.5`) as a
dev dependency and a test foundation: `phpunit.xml.dist`, a `tests/phpunit/`
directory with a bootstrap, a `composer test` script, and a starter suite.

### Scope

WordPress 0.71 is 2003-era procedural code with no test seams -- most of it
...
`tests/phpunit/includes/bootstrap.php` simply requires it (after setting a dummy
...
| File | Covers |
|---|---|
| `tests/phpunit/tests/HelpersTest.php` | `zeroise()`, `is_email()`, `mysql2date()` |
| `tests/phpunit/tests/TextFormattingTest.php` | `wptexturize()`, `balanceTags()` |
| `tests/phpunit/tests/SecurityTest.php` | `b2_csrf_token()` |

### Verification

`composer test` runs PHPUnit -- **12 tests, 18 assertions, all passing**.
phpcs and PHPStan are unchanged: both analyse `src/` only, and `tests/` is new
code outside their scope.

## Issue #59: Expand PHPUnit coverage toward all functions and classes

The Issue #53 starter suite covered five pure helpers and the CSRF token
...
Many legacy helpers read `global $wpdb` and the table-name globals
(`$tableposts`, `$tableusers`, ...). Two test-support classes make them
unit-testable without a live MySQL server:

- `tests/phpunit/includes/Support/FakeWpdb.php` -- a fake `$wpdb`. Its `get_row()` /
...
- `tests/phpunit/includes/Support/DatabaseTestCase.php` -- a shared base `TestCase` that, in
...
`tests/phpunit/includes/bootstrap.php` now also loads `b2template.functions.php`, `b2vars.php`
...
| `tests/phpunit/tests/HelpersTest.php` | `zeroise()`, `is_email()`, `mysql2date()` |
| `tests/phpunit/tests/TextFormattingTest.php` | `wptexturize()`, `balanceTags()` |
| `tests/phpunit/tests/SecurityTest.php` | `b2_csrf_token()`, `b2_csrf_field()` |
| `tests/phpunit/tests/FormattingFunctionsTest.php` | `wpautop()`, `autobrize()`, `unautobrize()`, `backslashit()`, `format_to_edit()`, `format_to_post()`, `popuplinks()`, `make_clickable()`, `strip_all_but_one_link()`, `make_url_footnote()`, `convert_bbcode()`, `convert_bbcode_email()`, `convert_gmcode()`, `convert_smilies()`, `convert_chars()`, `antispambot()` |
| `tests/phpunit/tests/DateAndMiscHelpersTest.php` | `date_i18n()`, `mysql2date()`, `get_weekstartend()`, `timer_start()`, `timer_stop()`, `addslashes_gpc()`, `wptexturize()` |
| `tests/phpunit/tests/TextileTest.php` | `textile()` (Textile 1.0 formatter), `callback_url()`, `linkit()`, `cmap()`, `encode_high()`, `decode_high()` |
| `tests/phpunit/tests/TemplateFunctionsTest.php` | `get_bloginfo()`, `get_the_title()`, `get_the_content()`, `get_the_excerpt()`, `single_month_title()`, `is_new_day()`, `apply_filters()`, `add_filter()` |
| `tests/phpunit/tests/DatabaseDependentFunctionsTest.php` | `get_postdata()`, `get_postdata2()`, `get_userdata()`, `get_userdata2()`, `get_userdatabylogin()`, `get_userid()`, `get_usernumposts()`, `user_pass_ok()`, `get_settings()`, `get_the_category()`, `get_the_category_by_ID()` |

Page-level scripts (`index.php`, `wp-admin/b2edit.php`, ...) and the `wpdb`
class against a live database are intentionally **out of scope** -- they are
covered by the E2E suite (Issue #60).

### Bug noticed (not fixed)

`user_pass_ok()` (`b2functions.php`) reads `$userdata['user_pass']` as an
...
`composer test` runs PHPUnit -- **95 tests, 150 assertions, all passing**
...
## Issue #60: Introduce an end-to-end (E2E) test suite

The PHPUnit suite (Issue #53) unit-tests pure helpers, but nothing
...
The project had no Node side before this Issue. Added at the repo root:

- `package.json` -- declares `@playwright/test` as the only devDependency and
  the `test:e2e` script (`playwright test`).
- `playwright.config.js` -- `testDir: ./tests/e2e`, baseURL `http://localhost:8080`,
...
`tests/e2e/helpers/` holds three helper modules:
...
Two spec files under `tests/e2e/`, **10 tests** in total:
...
The admin post-edit spec immediately caught a genuine PHP 8.3 regression:
...
With the Docker blog running (`docker compose up -d`), `npm run test:e2e`
...
## Issue #71: Add husky + lint-staged pre-commit hooks (phpcs / phpstan)

PR #66 merged a phpcs warning into `main` that went unnoticed until a
...
The hook was exercised with three commits: a file with a phpcs violation
...
## Issue #64: Remove the unused, buggy user_pass_ok()

`user_pass_ok()` in `b2-include/b2functions.php` ended with
...
## Issue #65: Prototype a custom block editor for WordPress 0.71

The Gutenberg investigation (Issue #61 / PR #63,
...
The editor loads a post via `load.php`, runs `parse()` on the
...
**Works:** loading a 0.71 post into a modern block editor, editing with the
...
`block-editor-prototype/node_modules/` and the build output
...
## Issue #76: Adopt the block editor into src/block-editor

The block editor prototype (Issue #65 / PR #70) was verified working
...
Rebuilt the React app (`npm run build` -> `src/block-editor/assets/`) and
...
## Issue #79: Build out the block-editor UI

The block editor at `src/block-editor/` mounted `@wordpress/block-editor`
...
### Backend (`api/load.php`, `api/save.php`)
...
Rebuilt the React app and verified end to end in a headless Chromium
...
After Issue #79's UI work, a follow-up bug surfaced: selecting text inside
...
本ドキュメントは WordPress 0.71-gold を PHP 8.3 で動作させるための変更を記録する。
移行の進行に合わせて Issue ごとに更新する。

---

## Issue #1: PHP 8.3 構文エラー(パースエラー)の修正

CLAUDE.md「戦略」の第一段階。`php -l` でパースエラー / Fatal error になる箇所を
...
波括弧による文字列オフセットアクセス構文は PHP 8.0 で廃止された。
...
関数呼び出し時の参照渡し(引数への `&`)は PHP 5.4 で廃止された。
...
全 52 PHP ファイルに対し `php -l` が `No syntax errors detected` を返すことを確認。
...
パースエラーではない PHP 8 の挙動変更(`wpdb` / `POP3` クラスの旧式
コンストラクタ、廃止された関数など)は、後続の互換性対応フェーズの Issue で扱う。

---

## Issue #5: 互換レイヤー方式による ext/mysql → mysqli 移行

WordPress 0.71 は PHP 7.0 で廃止された ext/mysql API(`mysql_*`)を使用する。
...
#### 新規ファイル: `src/b2-include/mysql-shim.php`

使用中の 16 個の `mysql_*` 関数を `mysqli` の薄いラッパーとして定義する。
...
ファイル先頭でシムを require する。旧式コンストラクタ `function wpdb(...)`
を `function __construct(...)` に改名する。旧式コンストラクタは PHP 8.0 で
コンストラクタとして認識されない。

### 検証

Docker の `web` コンテナ内で `db` コンテナに対しテストスクリプトを実行し、
...
`wp-db.php` の `define('OBJECT', ..., true)` は PHP 8.3 で `E_WARNING` を
...
## Issue #7: WordPress インストーラを PHP 8.3 + MySQL 8 で動作させる

最初の具体的・検証可能なマイルストーン — `wp-admin/wp-install.php` を一通り
...
WordPress 0.71 のインストーラ SQL は 2003 年頃の MySQL の寛容な挙動に依存
...
5 行目はインストーラのステップを `$HTTP_GET_VARS['step']` から読んでいた。
このスーパーグローバルは PHP 5.4 で廃止されたため `$step` は常に null となり、
インストーラは step 0 から進めなかった。`$_GET['step']` に変更した。

### 検証

Docker 環境で、空の DB から `wp-install.php?step=1` と `?step=2` がいずれも
...
他ファイルの `$HTTP_*_VARS`、`get_magic_quotes_gpc()`、`ereg`/`each` 等の
廃止関数、ブログ表示・管理画面の描画は後続 Issue で扱う。

---

## Issue #9: PHP 7/8 で廃止された関数・変数の置き換え

ブログ本体・管理画面の表示に向け、廃止された PHP 機能 4 種類に対応した。
コミットは項目ごとに分割している。
...
新規ファイル `src/b2-include/php-compat.php` に `false` を返す
...
POSIX 正規表現関数(PHP 7.0 で廃止)を `php-compat.php` で PCRE 上に再実装。
POSIX パターンは `~` デリミタで包み、PCRE エラー時は安全に縮退する。

#### 3. `each()` shim

`each()`(PHP 8.0 で廃止)を `php-compat.php` で `key()`/`current()`/`next()`
を用いて再実装。

#### 4. `$HTTP_*_VARS` -> superglobals

`$HTTP_*_VARS` スーパーグローバル(PHP 5.4 で廃止)を 28 ファイル・343 箇所で
`$_GET` / `$_POST` / `$_COOKIE` / `$_SERVER` へ機械的に置換。

#### 5. `array_merge()` null argument in `apply_filters()` (discovered during verification)

ブログ本体の表示検証で 5 件目の障壁が判明: `apply_filters()` が null の
...
Docker 環境(WordPress インストール済み)で、ブログ本体(`index.php`)と
`b2login.php` がいずれも HTTP 200 を返し、**fatal エラーが発生しない**ことを確認。

### 既知の残課題

ブログ本体は fatal エラーなく描画されるが、初期投稿はまだ表示されない。検証
...
## Issue #11: ブログ本体に投稿を表示させる

Issue #9 のフォローアップ。ブログが初期投稿を表示できない障壁が 2 つあった。

### 変更内容

#### 1. `src/blog.header.php` — PHP 8 comparison semantics

PHP 8.0 は文字列と数値の比較を変更し、`0 == ''` は false になった。条件
...
SQL を修正すると投稿はクエリされたが本文が空で描画された。
...
## Issue #13: 互換シムをネイティブ書き直しに置換し、静的解析を導入

静的解析ツールを導入し、その検出結果を受けて互換シム方式を全廃した。
...
PHPCompatibility(testVersion 8.3)の検出は **234 件から 14 件**へ減少し、
シム関連の検出は完全に解消。Docker 環境でインストーラが完走し、ブログ本体が
投稿を表示する。fatal エラー・SQL エラーは無く、互換シムも読み込まれない。

### スコープ外

実行時の `E_WARNING` / `E_DEPRECATED`(未定義変数、動的プロパティ、`${var}`
補間、`define()` 第3引数、ほとんど通らないパスの `/e` 修飾子、`POP3` 旧式
コンストラクタ)は、別の後続 Issue で扱う。

---

## Issue #15: ブログ本体の実行時警告を掃除

Issue #13 完了後もブログ本体は約 46 件の非 fatal な `E_WARNING` /
...
Docker 環境で `http://localhost:8080/` が HTTP 200 を返し、初期投稿を表示し、
`Warning` / `Deprecated` / `Notice` を **0 行**しか出さない。fatal エラー・
SQL エラーも無い。

### スコープ外

ブログ本体が通らない管理画面の警告、残りの `/e` 修飾子、`POP3` 旧式
コンストラクタは後続 Issue で扱う。

---

## Issue #19: b2register.php の 404 とログイン・フィードの警告を修正

Playwright のリンク検査でフロントのリンクは完全にクリーンと確認されたが、
...
フロントページの内部リンク 11 本を Playwright で巡回した結果、全リンクで
HTTP 200・fatal エラー無し・**PHP 警告 0**・**JS エラー 0**(`b2register.php` の
スタイルシート 404 も解消)。フロントは不変(引き続き警告 0)。

---

## Issue #22: phpcs と PHPStan(level 0) を 0 件にする

静的解析の仕上げ: phpcs を 11→0、PHPStan level 0 を 215→0 にし、PHPStan の
baseline を削除して、両ツールを抑制なしで完全にクリーンにする。

### 変更内容

phpcs(PHPCompatibility)検出、11 → 0:
...
XML-RPC エンドポイント(`xmlrpc.php`)には PHP 8 の*実行時*非互換が残る。
...
## Issue #24: 管理画面の実行時警告を修正

管理者でログインし WordPress 0.71 の管理画面を巡回した。致命的/構文
...
管理者でログインし、全管理画面 -- b2edit.php(新規/編集/存在しない投稿)、
...
ブログ本体の category / archive ページには既存のレガシー警告が 2 件残る
(`b2template.functions.php` の 825 行・105 行)。`main` にも存在し、フロント
エンドの実行時警告の別 Issue で扱う。

---

## Issue #26: フロントエンドの実行時警告 2 件を修正

Issue #24 の続き。ブログ本体(home・category・archive・単一投稿・検索・
...
フロントエンドの全ページ -- home・`?cat=`・`?m=`(年と年月)・`?p=`・
...
## Issue #28: 管理画面からの投稿でリダイレクトが失敗する

「管理画面から投稿」をテストした結果、投稿自体は行が INSERT されるが
...
管理画面から、投稿の作成(publish と draft)、編集(`editpost`、
...
## Issue #31: 数値 ID コンテキストの SQL インジェクションを修正

セキュリティ監査(Issue #31)で、WordPress 0.71 はすべての SQL クエリを
...
`mysqli` のプリペアドステートメント/バインドへの全面移行が理想的な長期
...
## Issue #32: HTML 出力に反映されるユーザー入力をエスケープ

セキュリティ監査(Issue #32)で、ユーザー制御データ(`$_SERVER` の値、
...
`the_content()` が描画する投稿本文、および `comment_text()` が描画する
...
## Issue #33: 状態変更を行う管理操作に CSRF 対策を追加

セキュリティ監査(Issue #33)で、WordPress 0.71 には CSRF 対策が一切無い
...
**公開コメントフォーム**(`b2comments.post.php`・`b2comments.php`・
...
WordPress 0.71-gold はパスワードを平文で保存・比較し、パスワード再発行で
...
新しいパスワードは `b2register.php`(登録)と `wp-admin/b2profile.php`
...
`checklogin()` は `wordpresspass` クッキーを `md5($userdata->user_pass)` と
...
`b2login.php` の `retrievepassword` ケースは `$user_data->user_pass` を
...
すべての認証 `setcookie()` 呼び出し(`b2login.php` の `wordpressuser`・
...
bcrypt ハッシュは 60 文字だが `b2users.user_pass` は `varchar(20)` だった。
...
- `src/b2login.php` — `b2_auth_cookie_flags()` ヘルパー。`login()` を
  ログイン名で照合し `password_verify()` で検証(レガシー行は再ハッシュ)
  するよう書き直し。認証クッキーを `md5(保存値)` に設定。ログアウト
  クッキーを強化。`retrievepassword` は保存された秘密情報をメール送信せず
  リセットする。
- `src/b2register.php` — 登録時に `password_hash()` の出力を保存する。
- `src/wp-admin/b2profile.php` — プロフィールのパスワード変更時に
  `password_hash()` の出力を保存し、強化したクッキーを `md5(保存ハッシュ)`
  に設定する。
- `src/wp-admin/wp-install.php` — `b2users.user_pass` カラムを
  `varchar(255)` に拡張する。

### 検証

変更した 4 ファイルすべてで `php -l` が通り、`composer phpcs` は違反 0 件、
...
`md5(パスワードハッシュ)` のベアラークッキーをサーバー側のランダムな
...
## Issue #44: XML-RPC・コメント・トラックバック・ピンバック機能を撤去

### 概要

WordPress 0.71-gold から XML-RPC サーバ・コメント機能・トラックバック・
...
各関数を削除する前に `src/` ツリー全体を grep し、残存する呼び出し元が
...
`b2functions.php` の `make_url_footnote()` は*残した* — XML-RPC ヘルパー
...
`php -l` は変更した全ファイルおよび `src/**/*.php` 全体で通る(構文
...
セキュリティ監査(Issue #35)で、管理画面の認可の適用が不統一であること
...
新しい最低レベルゲートは Issue #33 で追加した CSRF チェックの*後*に実行
...
`php -l` は変更した両ファイル(`b2team.php`・`b2edit.php`)で通る(構文
エラー 0)。`composer phpcs` は違反 0 件(41 ファイル)。
`composer phpstan --memory-limit=1G` はエラー 0 件(既定の 128 MB はこの
...
## Issue #36: ファイルアップロードのセキュリティ

セキュリティ監査(Issue #36)で、`wp-admin/b2upload.php` が保存先パスを
...
変更はすべて `wp-admin/b2upload.php` に限定する。
...
`move_uploaded_file()` は PHP 自身が正当な HTTP アップロードとして記録
...
`php -l wp-admin/b2upload.php` は構文エラー 0 で通る。`composer phpcs`
...
セキュリティ監査(Issue #37)で、深刻度が中程度の残り 3 件の問題が
...
**A. SQL エラーの露出。** `wp-admin/linkmanager.php` には小さなヘルパ
...
**B. バージョン露出。** `index.php` の `generator` meta タグは
...
**C. `register_globals` 風の `$$var` 代入。**

- `$$b2var` 代入ループは 11 個のエントリスクリプトに現れる:
  `blog.header.php`・`b2login.php`・`b2register.php`、および管理スクリプト
  `wp-admin/b2header.php`・`b2edit.php`・`b2categories.php`・`b2template.php`・
  `b2options.php`・`b2profile.php`・`b2team.php`・`linkmanager.php`・
  `linkcategories.php`。各ループは **グローバルスコープ** で動作する
  (関数内のものは無い)ため、そこでは `$$b2var` と `$GLOBALS[$b2var]` は
  完全に等価である。各 `$$b2var` の読み書き(`isset()` 判定を含む)を明示的な
  `$GLOBALS[$b2var]` 形へ置き換え、各ループの前に変更を説明する英日コメントを
  追加した。これは挙動を保つ可読性/堅牢性の強化である: スクリプトが既知の
  ホワイトリスト化されたグローバル群を `$_GET`/`$_POST` から生成することを
  明示し、フロントエンドの動作を変えずに脆い可変変数構文を除去する。

### 項目 C の判断

監査は項目 C を堅牢化するか、そのまま残すかの判断を求めていた。選んだ
...
`php -l` は変更した 17 ファイルすべてで構文エラー 0 で通る。`composer
...
## Issue #49: phpcs を WordPress コーディング規約に引き上げる

phpcs はこれまで `PHPCompatibility` のみを実行していた。Issue #49 で公式の
...
3 段階すべての後、`composer phpcs` は **エラー 0 件・警告 0 件** を報告する。

本 Issue の追加対応として、`PHPCompatibility` 標準(および
...
| Sniff | Count | 修正 |
|---|---|---|
| `WordPress.PHP.YodaConditions.NotYoda` | 232 | 各 `$var == literal` 比較を Yoda 形式 `literal == $var` に書き換え(挙動は同一) |
| `PSR2.Classes.PropertyDeclaration` (`VarUsed` / `ScopeMissing`) | 24 | `wpdb` のプロパティの `var` を明示的な `public` に置換 |
| `Squiz.Scope.MethodScope.Missing` | 13 | `wpdb` の各メソッドに明示的な `public` 可視性を付与 |
| `PSR2.ControlStructures.SwitchDeclaration` | 14 | `case`/`default` 本体を包む `{ }` ブロックを除去 |
| `Squiz.PHP.DisallowMultipleAssignments.FoundInControlStructure` | 9 | `if ( ... )` 条件内の代入を独立した文に移動 |
| `Squiz.ControlStructures.ControlSignature.SpaceAfterCloseBrace` | 4 | `elseif`/`else` を直前の閉じ波括弧に連結 |

### 除外した sniff

以下の sniff は `phpcs.xml.dist` の `<rule ref="WordPress-Core">` ブロック内
で除外する。いずれも改名・SQL 書き換え・挙動変更が必要で、コードスタイル対応の
範囲外である。

| Sniff | 除外理由 |
|---|---|
| `WordPress.NamingConventions.ValidVariableName` / `ValidFunctionName` | snake_case 改名は 2003 年当時の b2/WordPress API を書き換える |
| `PEAR.NamingConventions.ValidClassName` | `class wpdb` を `Wpdb` に改名要求。`wpdb` は公開 API クラス |
| `Universal.NamingConventions.NoReservedKeywordParameterNames` | 引数の改名は公開関数のシグネチャを変える |
| `WordPress.Files.FileName` | ファイル名の改名は全 include/require とリンクに波及 |
| `WordPress.DB.PreparedSQL` | prepared statement。Issue #31 で `(int)` キャストにより SQL は堅牢化済み |
| `WordPress.DB.RestrictedFunctions` | 「`mysqli_*` でなく `$wpdb`」だが 0.71 はデータ層そのもので設計上 mysqli を直接呼ぶ |
| `Universal.Operators.StrictComparisons` | `==`→`===` は比較の厳密さを変える |
| `WordPress.PHP.StrictInArray` | `in_array()` への `true` 追加は比較の厳密さを変える |
| `WordPress.DateTime.RestrictedFunctions` | `date()`→`gmdate()` はタイムゾーンの挙動変更 |
| `WordPress.PHP.NoSilencedErrors` | `@` の除去は実行時の警告挙動を変える |
| `Generic.CodeAnalysis.AssignmentInCondition` | `while ( $row = mysqli_fetch_*() )` の取得ループ慣用句は括弧追加では抑止できず、全ループ再構成は無限ループのリスク。安全な `if ( ... )` の事例は修正済み |

### 検証

`vendor/bin/phpcs` はエラー 0 件・警告 0 件。`php -l` は変更した 30 ファイル
...
## Issue #53: PHPUnit とテストスイートの土台を追加

WordPress 0.71 にはテストが無かった。Issue #53 で PHPUnit(`^12.5`)を
dev 依存として追加し、テストの土台を整える: `phpunit.xml.dist`、ブートストラップ
付きの `tests/phpunit/` ディレクトリ、`composer test` スクリプト、初期スイート。

### 範囲

WordPress 0.71 は 2003 年当時の手続き型コードでテストの接合点が無い
...
`b2functions.php` は DB 接続なしで単独読み込みできるため、`tests/phpunit/includes/bootstrap.php`
...
`composer test` で PHPUnit が走り、**12 テスト・18 アサーション・全合格**。
phpcs と PHPStan は不変: 両者は `src/` のみを解析し、`tests/` はその対象外の
新規コードである。

## Issue #59: PHPUnit の網羅性を高める

Issue #53 の初期スイートは純粋なヘルパー 5 つと CSRF トークンヘルパーを
...
ページレベルのスクリプト(`index.php`・`wp-admin/b2edit.php` ほか)と、実
データベースに対する `wpdb` クラスは意図的に**範囲外** -- E2E スイート
(Issue #60)で扱う。

### 発見したバグ(未修正)

`user_pass_ok()`(`b2functions.php`)は `$userdata['user_pass']` を
...
`composer test` で PHPUnit が走り、**95 テスト・150 アサーション・全合格**
(12 テスト・18 アサーションから増加)。`php -l` は新規テストファイルすべてで
構文エラー 0 件。phpcs と PHPStan は不変: 両者は本 Issue が触れていない `src/`
のみを解析し、`tests/` はその対象外である。

## Issue #60: E2E テストスイートを導入

PHPUnit スイート(Issue #53)は純粋なヘルパーを単体テストするが、2003 年
...
`tests/e2e/helpers/` に 3 つのヘルパーモジュールを置く:
...
`tests/e2e/` 配下に 2 つの spec ファイル、合計 **10 テスト**:
...
管理画面の投稿編集 spec が、本物の PHP 8.3 リグレッションを即座に検出した:
...
Docker ブログを起動した状態(`docker compose up -d`)で `npm run test:e2e`
...
## Issue #71: husky + lint-staged の pre-commit フックを追加

PR #66 が phpcs 警告に誰も気づかないまま `main` にマージし、マージ後の
...
フックは 3 つのコミットで検証した: phpcs 違反のあるファイル(コミット
ブロック)、phpcs はクリーンだが未定義関数を呼ぶファイル(phpstan が
ブロック)、クリーンなファイル(コミット成功)。`src/` のランタイムコードは
変更していない -- 本 Issue は開発ツールの追加のみである。

## Issue #64: 未使用かつ不具合のある user_pass_ok() を削除

`b2-include/b2functions.php` の `user_pass_ok()` は末尾が
...
## Issue #65: WordPress 0.71 向けカスタムブロックエディタを試作する

Gutenberg 調査(Issue #61 / PR #63、`docs/gutenberg-investigation.md`)の
...
エディタは `load.php` 経由で投稿を読み込み、`post_content` に `parse()` を
...
**動作する:** 0.71 の投稿をモダンなブロックエディタへ読み込む、標準の
...
`block-editor-prototype/node_modules/` とビルド成果物
...
## Issue #76: ブロックエディタを src/block-editor へ取り込む

ブロックエディタ試作(Issue #65 / PR #70)はローカルで動作を確認して
...
React アプリを再ビルドし(`npm run build` -> `src/block-editor/assets/`)、
...
## Issue #79: ブロックエディタ UI を拡充する

`src/block-editor/` のブロックエディタは `@wordpress/block-editor` を
...
React アプリを再ビルドし、使い捨ての Docker スタック(ポート 8081 /
...
Issue #79 の UI 作業の後、後続のバグが判明した: 段落ブロック内の
