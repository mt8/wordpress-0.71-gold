# Porting Gutenberg to WordPress 0.71

An evidence-based feasibility investigation (Issue #61) into whether the
...
**Not feasible.** Gutenberg is not a self-contained drop-in package: it is
...
**However**, this verdict is about porting *Gutenberg the application*
...
Gutenberg is developed as an independent project: a **monorepo of 80+
...
## The four pillars Gutenberg depends on

### 1. The WordPress REST API

The block editor is **dependent on the WordPress REST API**: it loads,
...
**0.71 provides:** Nothing. There is no REST API,
...
### 2. The `@wordpress/*` JavaScript and build stack

Gutenberg is a **React single-page application**. Its source is
...
**0.71 provides:** Almost no JavaScript at all. The
...
### 3. Server-side block registration

Blocks are registered on the server with `register_block_type()`, which
...
**0.71 provides:** No block concept at all, and only
...
### 4. The modern `wp-includes/` bootstrap

Gutenberg in core relies on the modern WordPress bootstrap: `wp-load.php`
...
**0.71 provides:** A 2003 b2/cafelog layout. As
...
| Gutenberg requires | First in WP | WordPress 0.71-gold (2003) |
|---|---|---|
| REST API endpoints (`/wp-json/wp/v2`) | 4.7 (2016) | None — RSS/RDF read-only XML only |
| `@wordpress/*` JS packages, React SPA | 5.0 (2018) | One `.js` file (`b2quicktags.js`); no React |
| npm / webpack / esbuild build tooling | 5.0 (2018) | None — no Node.js toolchain |
| Block model (`<!-- wp:* -->` markup) | 5.0 (2018) | Posts are raw HTML in a `<textarea>` |
| `register_block_type()` / `block.json` | 5.0 (2018) | No block concept |
| Action hooks (`add_action` / `do_action`) | 1.2 (2004) | None — only primitive `add_filter` |
| `wp-includes/` + `wp-load.php` bootstrap | modern WP | `b2-include/` + `blog.header.php` |
| `wp_enqueue_script` / script modules | 2.6+ (2008) | None — hand-written `<script>` tags |
| Minimum PHP | 7.4+ | Written for PHP 4 (this repo runs it on 8.3) |

## Why this is not a "port"

A port moves working software onto a platform that already offers the
...
## An alternative: a custom block editor

Porting Gutenberg-the-application is not feasible (above), but the block
...
# Gutenberg を WordPress 0.71 へ移植する

Gutenberg(ブロックエディタ)を WordPress 0.71-gold へ移植できる/動かせる
かを、根拠に基づいて調査したもの(Issue #61)。wp-env の調査(Issue #55、
`docs/docker-environment.md` に記録)と同種の調査である。

## 結論

**実現不可能。** Gutenberg は単独で組み込める独立パッケージではない。
...
**ただし**、この判定は *アプリケーションとしての Gutenberg*(`wp-admin` の
...
Gutenberg は独立したプロジェクトとして開発されている。**`@wordpress/*`
...
## Gutenberg が依存する 4 本柱

### 1. WordPress REST API

ブロックエディタは **WordPress REST API に依存する**。投稿の読み込み・
...
**0.71 が提供するもの:** 何もない。REST API も JSON 出力もルーティング層も無い。`src/` を
...
### 2. `@wordpress/*` の JS・ビルドスタック

Gutenberg は **React のシングルページアプリケーション**である。ソースは
...
**0.71 が提供するもの:** JavaScript はほぼ皆無。`src/` ツリー全体に `.js` ファイルは **1 個**
...
ブロックはサーバー側で `register_block_type()` により登録される。これは
...
**0.71 が提供するもの:** ブロックの概念は皆無で、Plugin API も *原始的な前身*しかない。WordPress
...
### 4. モダンな `wp-includes/` ブートストラップ

コアの Gutenberg はモダンな WordPress ブートストラップに依存する。
...
**0.71 が提供するもの:** 2003 年の b2/cafelog 構成である。wp-env 調査で既に確認したとおり、
...
| Gutenberg の要件 | WP 初出 | WordPress 0.71-gold (2003) |
|---|---|---|
| REST API endpoints (`/wp-json/wp/v2`) | 4.7 (2016) | 無し — RSS/RDF の読み取り専用 XML のみ |
| `@wordpress/*` JS packages, React SPA | 5.0 (2018) | `.js` は 1 個のみ(`b2quicktags.js`)、React 無し |
| npm / webpack / esbuild build tooling | 5.0 (2018) | 無し — Node.js ツールチェーン無し |
| Block model (`<!-- wp:* -->` markup) | 5.0 (2018) | 投稿は `<textarea>` 内の生 HTML |
| `register_block_type()` / `block.json` | 5.0 (2018) | ブロックの概念無し |
| Action hooks (`add_action` / `do_action`) | 1.2 (2004) | 無し — 原始的な `add_filter` のみ |
| `wp-includes/` + `wp-load.php` bootstrap | モダン WP | `b2-include/` + `blog.header.php` |
| `wp_enqueue_script` / script modules | 2.6+ (2008) | 無し — `<script>` タグ手書き |
| 最低 PHP | 7.4+ | PHP 4 向け(本リポジトリは 8.3 で動かす) |

## これが「移植」でない理由

移植とは、必要なインターフェースを既に備えたプラットフォームへ、動作する
...
アプリケーションとしての Gutenberg の移植は実現不可能(上記)だが、ブロック
